...
function setTime() {
  ...
  document.getElementById("ctext").innerText = d;
  var t = setTimeout(setTime, 900);
}
...
function loadTasksFromText(text) {
  ...
  for (var i = 0; i < text.length; i++) {
    if (text.charAt(i) == separator) {
      var newTask = text.substring(lastTaskEndPos, i);
      addItemTodo(newTask);
      lastTaskEndPos = i + 1;
    }
  }
...
function updateSyncedTasks(newTasks) {
  tasksText = newTasks;
  chrome.storage.sync.set({ "tasks": newTasks }, function () { });
}

// User clicked add button
// If text inside, add text to list
...
function removeItem() {
  ...
  updateSyncedTasks(tasksText.replace(item.innerText + separator, ""));
...
function addItemTodo(text) {
  ...
  updateSyncedTasks(tasksText + text + separator)
}
